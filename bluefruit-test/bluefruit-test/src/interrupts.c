...
void (*interrupts_uart_handler)(void);
...
//				| XPAR_AXI_AC97_0_INTERRUPT_MASK));
	...
	XIntc_EnableIntr(XPAR_INTC_0_BASEADDR,
		(XPAR_PIT_0_MYINTERRUPT_MASK | XPAR_PUSH_BUTTONS_5BITS_IP2INTC_IRPT_MASK
				| XPAR_AXI_AC97_0_INTERRUPT_MASK | XPAR_AXI_UARTLITE_0_INTERRUPT_MASK));
...
void interrupts_register_handler(uint8_t type, void (*func)(void)) {
	...
	else if (type == INTS_UART) interrupts_uart_handler = func;
}
...
void interrupt_handler_dispatcher(void* ptr) {
	...
	}else if (status & XPAR_AXI_UARTLITE_0_INTERRUPT_MASK) {
		// Let the audio controller know we got an interrupt!
		if (interrupts_uart_handler) interrupts_uart_handler();
		xil_printf("uart!\n\r");
